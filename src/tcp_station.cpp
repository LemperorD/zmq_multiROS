...
ros::Publisher* fov_pub_ = nullptr; // 飞机到地面：相机视场角
...
void fov_bridge_cb(int ID, ros::SerializedMessage &m); // 飞机到地面：相机视场角
...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < drone_num_; ++i) {
    ...
    fov_pub_[i] = nh.advertise<visualization_msgs::Marker>("/drone_"  + std::to_string(i) +"/fov" , 10);
  }
  ...
  for (int i = 0; i < drone_num_; ++i) {
    ...
    if (bridge->register_callback(i, "/fov_tcp_" + std::to_string(i), fov_bridge_cb))
    {
      ROS_INFO("Register fov callback for drone %d", i);
    }
  }
...
void fov_bridge_cb(int ID, ros::SerializedMessage &m) {
  visualization_msgs::Marker fov_msg_;
  ros::serialization::deserializeMessage(m, fov_msg_);
  fov_pub_[ID].publish(fov_msg_);
}